param(
    [string]$PATH_TO_ZROK="C:\temp\zrok\zrok.exe",
...
if ($IS_RESERVED) {
	Write-Host -ForegroundColor Blue "zrok foundry logs written to: $env:TEMP\zrok-foundry-logs.txt"
	Write-Host "Access FountdryVTT at https://$RESERVED_SHARE.share.zrok.io"
	Start-Process -FilePath "$PATH_TO_ZROK" -ArgumentList "share reserved $RESERVED_SHARE --headless" -RedirectStandardError "$env:TEMP\zrok-foundry-logs.txt" -NoNewWindow -Wait
} else {
